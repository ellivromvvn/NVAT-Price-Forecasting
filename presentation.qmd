...
`{r} #| label: diagram-workflow #| echo: false #| fig-width: 6  workflow <- DiagrammeR::grViz("digraph {  graph [layout = dot, rankdir = LR]  # define the global styles of the nodes. We can override these in box if we wish node [shape = rectangle, style = filled, fillcolor = Linen]  B [label = 'Tidy'] C [label = 'Visualize'] D [label = 'Estimate'] E [label = 'Evaluate'] F [label = 'Forecast']  # edge definitions with the node IDs B -> C -> D -> E -> F; }")  vtree::grVizToPNG(workflow, filename = "pictures/workflow.png", width = 600)}`
