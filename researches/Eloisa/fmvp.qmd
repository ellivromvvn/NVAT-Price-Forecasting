...
### Time Series of the Monthly Vegetable Prices
...
#| tbl-cap: "Summary of the Time Series of Monthly Vegetable Prices"
#| echo: false
...
For the carrots time series, the minimum price is `r tbl_all$Min[tbl_all$Vegetable == 'carrots']` in May 2020 and the maximum price is `r tbl_all$Max[tbl_all$Vegetable == 'carrots']` in July 2023. The average price is `r tbl_all$Mean[tbl_all$Vegetable == 'carrots']`. It has a standard deviation of `r tbl_all$SD[tbl_all$Vegetable == 'carrots']`.
...
#| label: tbl-stl
#| tbl-cap-location: top
#| tbl-cap: "STL Features of the Time Series of Monthly Vegetable Prices"
#| echo: false
...
The broccoli time series has a trend strength of `r tbl_stl$'Trend Strength'[tbl_stl$Vegetable == 'broccoli']`, showing that `r tbl_stl$'Trend Strength'[tbl_stl$Vegetable == 'broccoli']*100`% of the time series is accounted by trend not considering seasonality. It has an upward trend. On the other hand, it has a seasonal strength of `r tbl_stl$'Seasonal Strength'[tbl_stl$Vegetable == 'broccoli']`, showing that `r tbl_stl$'Seasonal Strength'[tbl_stl$Vegetable == 'broccoli']*100`% of the time series is accounted by seasonality not considering trend. Its peak is in `r tbl_stl$'Seasonal Peak'[tbl_stl$Vegetable == 'broccoli']` and its trough is in `r tbl_stl$'Seasonal Trough'[tbl_stl$Vegetable == 'broccoli']`.
...
The time series cross-validation yielded 360 models; 180 ARIMA and 180 ETS. That is 15 ARIMA models and 15 ETS models for each vegetable. Please see Appendix E for the full list of these models.
...
@tbl-fit shows the performance of ARIMA and ETS for each vegetable.
...
fc_carrots <- fit_carrots |>
  forecast(h = 4)
```

```{r}
tbl_carrots <- fc_carrots |>
  hilo() |>
  unpack_hilo(c(`80%`, `95%`)) |>
  as_tibble() |>
  group_by(date) |>
...
tbl_ljb_carrots <- augment(fit_carrots) |>
  filter(.model == "ARIMA") |>
  features(.innov, ljung_box, lag = 12, dof = 3) |>
  mutate(Model = fit_carrots$ARIMA) |>
  select(
    Vegetable = vegetable,
    Model,
    "LB Statistics" = lb_stat,
